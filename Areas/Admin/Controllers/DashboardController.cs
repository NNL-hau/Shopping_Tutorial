...
namespace Shopping_Tutorial.Controllers
{
    ...
    public class DashboardController:Controller
    {
        ...
        {
            var chartData = await _dataContext.Statisticals
                .Select(s => new
                {
                    date = s.DateCreated.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture),
 // Sửa thành DateCreated
                    sold = s.Sold,
                    quantity = s.Quantity,
                    revenue = s.Revenue,
                    profit = s.Profit
                })
                .ToListAsync(); // Sử dụng ToListAsync() cho hiệu suất tốt hơn

            return Json(chartData); // Trả về chartData thay vì data
        }
        ...
        public IActionResult FilterData(DateTime? fromDate, DateTime? toDate)
        {
            ...
            if (toDate.HasValue) // Đã sửa lại điều kiện đúng
            {
